<?php
  require_once 'dbconfig.php';
  $a = checkSalt($_GET['q']);
  if($a == null || !isset($a) || $a == 0 || $a == ''){
    header('Location: http://faithfulscholars.com/member_site');
    exit();
  } 
?>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title></title>
    <link rel="stylesheet" href="/member_site/stylesheet/app.css">
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-WskhaSGFgHYWDcbwN70/dfYBj47jz9qbsMId/iRN3ewGhXQFZCSftd1LZCfmhktB" crossorigin="anonymous">
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.min.js" integrity="sha384-smHYKdLADwkXOn1EmN1qk/HfnUcbVRZyYmZ4qpPea6sjB/pTJ0euyQp0Mk8ck+5T" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.bundle.min.js" integrity="sha384-u/bQvRA/1bobcXlcEYpsEdFVK/vJs3+T+nXLsBYJthmdBuavHvAW6UsmqO2Gd/F9" crossorigin="anonymous"></script>
  </head>
  <body>
    <div class="row outer-row">
      <div class="col-md-12">
        <img class="logoimg" src="/member_site/assets/faithfulscholarslogo.png" alt="">
        <header>
          <hr>
        </header>
        <div class="container">
          <div class="row">
            <div class="col-md-6 mx-auto">
              <h3 class="text-center">Password Reset</h3>
              <p>Welcome to the membership login for Faithful Scholars. Please enter your membership login credentials below to access the members only area.
            </div>
          </div>
          <br>
          <div class="row">
            <div class="col-md-4 mx-auto">
              <form class="" name="form1" method="post" action="resetpassword.php">
                <div class="form-group">
                  <label class="left" for="username">Email:</label>
                  <input class="form-control" name="myemail" type="text" id="myemail">
                </div>
		<div class="form-group">
		  <label class="left" for="mypassword">New Password:</label>
		  <input class="form-control" name="mypassword" type="password" id="mypassword">
		</div>
                <div class="text-center">
                  <input class="btn btn-primary" type="submit" name="Submit" value="Submit">
                </div>
              </form>
            </div>
          </div>
          <footer><small>&copy;2018 FaithfulScholars.</small></footer>
      </div>
    </div>
  </body>
</html>
